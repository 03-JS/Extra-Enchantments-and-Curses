...
public class Electrified extends Enchantment {
    ...
    public int getProtectionAmount(int level, DamageSource source) {
        return source == DamageSource.LIGHTNING_BOLT ? 4 : 0;
    }
        ...
        if (attacker instanceof LivingEntity) {
            if (user.world.isThundering()) {
                ...
                if (rng <= 2 && hitsAttacker == 1) {
                    LightningEntity lightningEntity = EntityType.LIGHTNING_BOLT.create(attacker.world);
                    assert lightningEntity != null;
                    lightningEntity.refreshPositionAfterTeleport(attacker.getX(), attacker.getY(), attacker.getZ());
                    attacker.world.spawnEntity(lightningEntity);
                }
                if (hitsAttacker == 4) {
                    hitsAttacker = 0;
                }
