        ...
        if (target instanceof HostileEntity || target instanceof PlayerEntity || target instanceof HoglinEntity || target instanceof BeeEntity
            ...
            if (hits == 7) {
                if (((LivingEntity) target).getDamageTracker().getMostRecentDamage() != null) {
                    target.damage(DamageSource.MAGIC, ((LivingEntity) target).getDamageTracker().getMostRecentDamage().getDamage() + 14);
                ...
                hits++;
                if (user.getHealth() <= 1) {
                    user.damage(DamageSource.MAGIC, 100);
                } else {
                    user.setHealth(user.getHealth() - 1);
                }
                user.world.playSound(null, user.getBlockPos(), SoundEvents.PARTICLE_SOUL_ESCAPE, SoundCategory.MASTER, 3f, 1f);
            }
        }
    }
}
