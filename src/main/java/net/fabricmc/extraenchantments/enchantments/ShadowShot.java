...
public class ShadowShot extends Enchantment {
    ...
    public void onTargetDamaged(LivingEntity user, Entity target, int level) {
        if (target instanceof LivingEntity && user.world.isNight() && ((LivingEntity) target).isDead() && !user.world.isThundering()) {
            int rng = (int) (Math.random() * 15);
            if (rng <= 2) {
                if (((LivingEntity) target).getRecentDamageSource().isProjectile()) {
                    user.world.playSound(null, user.getBlockPos(), SoundEvents.BLOCK_RESPAWN_ANCHOR_CHARGE, SoundCategory.MASTER, 1f, 1f);
